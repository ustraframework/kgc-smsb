  ...
  <div class="flex flex-col items-center my-[20px]">
    <TabView class="fill_tab industry_custom_tab" v-model:activeIndex="activeTab">
      ...
      <TabPanel header="멤버스 혜택">
        <div class="tabInner__title border-b">
          <h2>멤버스 혜택</h2>
          <p>
            정관장 멤버스 회원만이 누릴 수 있는 혜택!
            <br />
            언제, 어디서나 일상 속에서 혜택을 플러스해보세요.
          </p>
        </div>
            ...
            <TabPanel v-for="(tab, i) in rankTab" :index="i">
              <template #header>
                <div class="flex flex-col items-center gap-[4px]">
                  <Avatar :image="activeRankTab === i ? tab.img_select : tab.img_unselect" size="large" />
                  <span
                    class="text-[13px] font-bold leading-[15.6px] mt-[8px] text-[#C3C3C3]"
                    :style="{ color: activeRankTab === i ? tab.color : '' }"
                  >
                    {{ tab.label }}
                  </span>
                </div>
