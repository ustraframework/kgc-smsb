  ...
  <div class="flex flex-col items-center w-[650px]">
    ...
    <div class="w-full flex flex-col items-center mt-[40px] gap-[40px]">
      ...
      <div class="w-full flex pb-[40px] border-b border-[#E7E7E7] gap-[20px]">
        ...
        <div class="flex items-center">
          <RadioButton class="p-radiobutton-box-lg" v-model="gender" inputId="female" name="female" value="female" />
          <label for="female">여자</label>
        </div>
      </div>

      <!-- 날짜 -->
      <div class="w-full flex items-center pb-[40px] border-b border-[#E7E7E7] gap-[20px]">
        <Calendar inputClass="!h-[50px]" v-model="date" placeholder="2023-01-01" />
        <div class="flex items-center">
      ...
      <div class="w-full flex gap-[8px]">
        <Button class="w-2/4" size="large" label="취소" outlined @click="$router.push('/pubs/MP/MI/UI_FU_0024_1')" />
    ...
    <Dialog v-model:visible="isShowConfirm" modal :style="{ width: '500px' }">
      ...
      <template #footer>
        <div class="flex justify-center pt-[24px] pb-[30px] px-[30px] gap-[10px]">
          <Button label="확인" @click="$router.push('/pubs/MP/MI/UI_FU_0024')" />
        </div>
      </template>
    </Dialog>
  </div>
