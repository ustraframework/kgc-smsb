<template>
  <div class="flex flex-col items-center w-[650px]">
    <h1 class="text-[30px] font-medium mb-[30px]">포인트 조회</h1>
    ...
    <div class="w-[650px] bg-[#F4F6FA] p-[24px] rounded-[12px]">
      <div class="flex flex-col justify-center items-center mb-[20px] border-b border-[#E7E7E7] pb-4">
        ...
        <span class="text-[30px] text-[#D20F27] mt-2">
      ...
      <div class="flex justify-between items-center mb-4">
        <div class="flex items-center">
          <span class="text-[15px] font-medium mr-4">특별 포인트</span>
          <span class="text-[20px]">
            <strong class="mr-0.5">500</strong>P
          </span>
        </div>
        <div class="flex items-center cursor-pointer" @click="$router.push('/pubs/MP/PI/UI_FU_0019_1')">
          <span class="text-[15px] font-medium">
            당월 소멸예정
          </span>
          <span class="text-[12px]">(전일기준)</span>
          <img src="@/assets/images/svg/ico_more.svg" class="mr-4" alt="더보기"/>
          <span class="text-[20px]">
            <strong class="mr-0.5">1,000</strong>P
          </span>
        </div>
    ...
    <div class="w-full flex justify-between mt-[40px] items-center">
      <span class="text-[15px]">
        {{ `${selectedValue ? `최근 ${selectedValue}개월 ∙ `: ''}전체` }}
      </span>
    ...
    <div class="w-full flex flex-col rounded-[12px] border border-[#E7E7E7] mt-5 px-8 py-2">
      <div v-for="(item, i) in list" :key="i" class="flex justify-between py-6" :class="[i !== 0 ? ' border-t border-[#E7E7E7]' : '']">
        <div class="flex flex-col">
          ...
          <span class="text-[14px] text-[#666666] mt-2.5">{{item.date}}</span>
        </div>
        <div class="flex flex-col text-right" :class="[item.pointType === 'plus' ? 'text-[#D20F27]' : '']">
          <span class="text-[14px] font-normal">{{item.type}}</span>
          <span class="text-[18px] mt-0.5">
